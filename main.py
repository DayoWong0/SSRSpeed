...
fileHandler = logging.FileHandler("./logs/" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".log",
                                  encoding="utf-8")
...
if (__name__ == "__main__"):
    ...
    print("Press ENTER to conitnue or Crtl+C to exit.")
    ...
    logger.debug(
        "\nFilter keyword : %s\nFilter group : %s\nFilter remark : %s\nExclude keyword : %s\nExclude group : %s\nExclude remark : %s" % (
            str(FILTER_KEYWORD), str(FILTER_GROUP_KRYWORD), str(FILTER_REMARK_KEYWORD), str(EXCLUDE_KEYWORD),
            str(EXCLUDE_GROUP_KEYWORD), str(EXCLUDE_REMARK_KEWORD)
        )
    )
    ...
    if not SKIP_COMFIRMATION:
        ...
        print("Before the test please confirm the nodes,Ctrl-C to exit. (Y/N)")
        ans = "Y"
        if ans.upper() == "Y":
            pass
        else:
